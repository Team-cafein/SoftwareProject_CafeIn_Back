...
const askGpt = async (userMessage) => {
  try {
    ...
    const formatData = (data) => {
      ...
      const dataMessages = templateStringArray.map((formattedData, index) => ({
        role: 'system',
      ...
      const addDataArray = [
        {
          role: 'system',
          content: `마지막 반환해주는 url의 /{cafe}/{cafeid}/{beverage} 순서는 절대적으로 지켜져야한다. 가끔 반대로 /{beverage}/{cafeid} 순서로 나올 때가 있는데 이러면 안된다. cafeid는 1부터 5까지 밖에 없기 때문에 만약 5을 넘는다면 틀린 것이므로 /{cafe}/{cafeid}/{beverage}경우로 반환해주어야 한다.`,
        },
        {
          role: 'system',
...
const extractCafeAndBeverageName = (userMessage) => {
  const beverageKeywords = ['아메리카노', '콜드브루', "콜드", "콜드 브루", '라떼', '에이드', '주스', '티', "요거트"];
