...
// GPT-3 API에 사용자 질문을 전달하고 응답을 받아오는 함수
const askGpt = async (userMessage) => {
  try {
    ...
    let messages = [
      ...
      },
      {
        ...
        content:
          // 'tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 americano,latte,ade,juice,tea 아메리카노, 라떼, 에이드, 주스, 티 인지 종류를, tag[2]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[3]은 ice,hot 아이스 또는 핫, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자의 요청에 따라 태그를 파악해 적절한 음료를 추천해야한다.',
          'tag가 5개인 경우 tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 ice,hot 아이스 또는 핫, tag[2]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[3]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[4]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자의 요청에 따라 태그를 파악해 적절한 음료를 추천해야한다.'
      },
      {
        role: 'system',
        content:
          'tag가 6개인 경우 tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 americano,latte,ade,juice,tea 아메리카노, 라떼, 에이드, 주스, 티 인지 종류를, tag[2]은 ice,hot 아이스 또는 핫, tag[3]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자의 요청에 따라 태그를 파악해 적절한 음료를 추천해야한다.',
          // 'tag가 5개인 경우 tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 latte,ade,juice, smoothie, tea 라떼, 에이드, 주스, 스무디, 티 인지 종류를, tag[2]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[3]은 ice,hot 아이스 또는 핫, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자의 요청에 따라 태그를 파악해 적절한 음료를 추천해야한다.'
      },
      {
        role: 'system',
        content:
      ...
      },
      {
        role: 'system',
        content:
          'tag가 6개인 경우 tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 americano,latte,ade,juice,tea 아메리카노, 라떼, 에이드, 주스, 티 인지 종류를, tag[2]은 ice,hot 아이스 또는 핫, tag[3]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자한테 말할 때에는 키워드를 한국말로 치환해서 알려주어야한다.',
          // 'tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 latte,ade,juice,smoothie,tea 라떼, 에이드, 주스, 스무디, 티 인지 종류를, tag[2]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[3]은 ice,hot 아이스 또는 핫, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자한테 말할 때에는 키워드를 한국말로 치환해서 알려주어야한다.'
      },
      {
        role: 'system',
        content:
          // 'tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[1]은 americano,latte,ade,juice,tea 아메리카노, 라떼, 에이드, 주스, 티 인지 종류를, tag[2]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[3]은 ice,hot 아이스 또는 핫, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자한테 말할 때에는 키워드를 한국말로 치환해서 알려주어야한다.',
          'tag가 5개인 경우 tag[0]은 coffee,non-coffee 커피인지 논커피인지, tag[2]은 ice,hot 아이스 또는 핫, tag[3]은 k_low,k_mid,k_high 칼로리가 낮음 중간 높음, tag[4]은 s_low, s_mid, s_high 당도가 낮음, 중간, 높음을, tag[5]은 p_low,p_mid,p_high 가격 싸다 중간 비싸다를 나타내며 사용자한테 말할 때에는 키워드를 한국말로 치환해서 알려주어야한다.'
      },
      {
        role: 'system',
        content:
      ...
      },
      {
        role: 'system',
        content:
          '위의 주어진 형식과 같은 assistant의 데이터가 들어가 있다면 assistant의 형식에 맞춰 있는 데이터를 먼저 보여줘야하고, 위의 주어진 형식의 assistant형식에 데이터가 안들어 온다면 그 것은 DB에서 검색하지 못한 데이터 이므로 이때는 콕인(CokIn)이 알고 있는 음료 정보를 보여주도 된다.',
      },
      // {
      //   role: 'system',
      //   content: `데이터 형식을 받으면 메뉴를 추천할 때의 콕인(CokIn)의 대답 형식은 사용자가 원하는 메뉴를 이름과 tag 등으로 판단하고 추천해주는 답변일 시 마지막에는 무조건 http://localhost:3000/category/{cafe}/{cafeid}/{beverage} 형식으로 대답해 주어야 한다. {}는 보간법으로 {}안에는 실제로는 {cafe}는 카페이름[starbucks, ediya, hollys, paik, mega]의 이름 중에만 {cafeid}는 [1, 2, 3, 4, 5,] 중 에만 {beverage}는 실제 저장된 음료 번호(정수)가 매칭되서 들어가야한다 `,
    ...
    // Add these lines at the beginning of the askGpt function
    ...
    //   cafeName = 'all';
    // }
    if ((cafeName == '' || !cafeName) && beverageName && tag) {
      cafeName = 'all';
    } else if ((cafeName == '' || !cafeName) && !beverageName && tag) {
      cafeName = 'all';
    } else if ((cafeName == '' || !cafeName) && beverageName && !tag) {
      cafeName = 'all';
    }
    ...
    const formatData = (data) => {
      ...
      const templateStringArray = data.map(
        (item) =>
          `id: ${item.id}, cafeid: ${item.cafeid}, beverage: ${
            item.beverage
          }, name: '${item.name}', cafe: '${item.cafe}', content: '${
            item.content
      ...
      const additionalSystemMessages = [
        ...
        {
          role: 'system',
          content: `콕인(CokIn)이 ${data[0].cafe}에서 음료를 추천해드려요! <a href="http://localhost:3000/category/${data[0].cafe}/${data[0].cafeid}/${data[0].beverage}" target="_blank">[${data[0].name} 바로가기]</a> 앞에 <a> 태그 형식으로만 글자 하나 안틀리고 반환해야한다. 음료 이미지 \n<div style={{width: "30px", height: "40px"}}><img className="img" src=${data[0].image} alt="Image" /></div>\n ${data[0].name} ${data[0].content}을 요약한 내용. ${data[0].tag}에 대한 내용 아래 링크에서 더 자세한 정보를 확인해보세요. ${data[0].image}는 url 즉, 이미지 url를 반환하고, 실제로 콕인이 밑에 데이터 중 최적의 답을 골라서 대입해서 표현하면 된다. url과 image url 은 인식될 때 구분 될 수 있어야하므로 url은 처음에 image url은 마지막에 표현한다.`,
        },
        {
          role: 'assistant',
        ...
        {
          role: 'system',
          content: `데이터 베이스에 저장된 데이터라면 항상 위에 어시스턴트로 지정한 <a/> 태그가 들어있는 [${data[0].name} 바로가기]을 보여주어야하며, 항상 가장 마지막에 메뉴 이미지 ${data[0].image} 형식으로 이미지 링크 url를 보여주어야한다. 만약 여려개의 음료를 추천했으면 그 중의 하나의 이미지만 표현해야한다.`,
        },
      ];
    ...
    if (cafeName && !beverageName && !tag) {
      ...
      // 음료 이름만 있는 경우
      const cafeDataBeverage = await getCafeDataByName(cafeName, beverageName);
      messages = messages.concat(formatData(cafeDataBeverage));
      console.log(messages);
    } else if (!cafeName && !beverageName && tag) {
      // 태그만 있는 경우
      const cafeDataTag = await getCafeDataByTag(cafeName, tag);
...
const extractCafeName = (userMessage) => {
  const cafeKeywords = ['스타벅스', '이디야', '할리스', '빽다방', '메가'];
...
const extractBeverageName = (userMessage) => {
  ...
  // 먼저 긴 키워드부터 체크하도록 변경
  beverageKeywords.sort((a, b) => b.length - a.length);
  ...
  for (const keyword of beverageKeywords) {
    if (userMessage.includes(keyword)) {
      ...
      if (keyword == '아메리카노' || keyword == '아메') {
        ...
        extractedBeverageName = '요거트';
      } else if (keyword == '에스프레소') {
        extractedBeverageName = '에스프레소';
      }
      return extractedBeverageName;
    }
  }
...
//       let extractedCafeTag;
//       if (keyword == '커피') {
//         extractedCafeTag = 'coffee';
//       } else if (keyword == '논커피') {
//         extractedCafeTag = 'non-coffee';
//       } else if (keyword == '아이스') {
//         extractedCafeTag = 'ice';
//       } else if (keyword == '핫') {
//         extractedCafeTag = 'hot';
...
//       } else if (keyword == '티') {
//         extractedCafeTag = 'tea';
//       } else if (keyword == '칼로리 낮' || keyword == '칼로리가 낮') {
//         extractedCafeTag = 'k_low';
//       } else if (keyword == '칼로리 중' || keyword == '칼로리가 중') {
//         extractedCafeTag = 'k_mid';
//       } else if (keyword == '칼로리 높' || keyword == '칼로리가 높') {
//         extractedCafeTag = 'k_high';
//       } else if (keyword == '당도 낮' || keyword == '당도가 낮') {
//         extractedCafeTag = 's_low';
//       } else if (keyword == '당도 중' || keyword == '당도가 중') {
//         extractedCafeTag = 's_mid';
//       } else if (keyword == '당도 높' || keyword == '당도가 높') {
//         extractedCafeTag = 's_high';
//       } else if (keyword == '가격 낮' || keyword == '가격이 낮') {
//         extractedCafeTag = 'p_low';
//       } else if (keyword == '가격 중' || keyword == '가격이 중') {
//         extractedCafeTag = 'p_mid';
//       } else if (keyword == '가격 높' || keyword == '가격이 높') {
//         extractedCafeTag = 'p_high';
//       }
//       // else {
//       //   extractedCafeTag = 'all';
